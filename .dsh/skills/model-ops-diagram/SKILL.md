...
Reference artifacts in the workspace: `qsa_ops.mmd`, `make_qsa_ops_diagram.py` (same diagram as hand-laid eddx/SVG). The `/home/jiali/codes/model_structure` session workspace is the grown-out example: every model folder holds `{model_slug}_structure.{mmd,svg,png,eddx}` + `{model_slug}_structure_preview.html` + its `config.json` and modeling source, and its root `index.html` lists them all (Qwen/Qwen3.8-Flash-Next — 48-layer hybrid, 70 boxes; zai-org/GLM-5.3-Flash — mHC + KDA + MLA/DSA, 61 boxes; deepseek-ai/DeepSeek-V4-Pro-0813 — HCA/CSA compressors + Lightning Indexer, 48 boxes; moonshotai/Kimi-K3 — MLA/KDA + attention residuals + latent MoE, 51 boxes; deepseek-ai/DeepSeek-V4.1-Flash — shared compressors + Engram + DSpark, 47 boxes).
